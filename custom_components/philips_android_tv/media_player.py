...
PHILIPS_TV_DOMAIN = "philips_android_tv"
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    hass.data.setdefault(PHILIPS_TV_DOMAIN, {})
    ...
    if hass.services.has_service(PHILIPS_TV_DOMAIN, SERVICE_SEND_REMOTE_KEY):
        return

    def service_send_remote_key(service):
        """Dispatch service c alls to target entities."""
        cmd = service.data.get(ATTR_COMMAND)
        entity_id = service.data.get(ATTR_ENTITY_ID)
        target_devices = [
            dev
            for dev in hass.data[PHILIPS_TV_DOMAIN].values()
            if dev.entity_id in entity_id
        ]

        for target_device in target_devices:
            target_device.send_key(cmd)

    hass.services.register(
        PHILIPS_TV_DOMAIN,
        SERVICE_SEND_REMOTE_KEY,
        service_send_remote_key,
        schema=SERVICE_SEND_REMOTE_KEY_SCHEMA
    )
